...
def contourValue(data, rowcoords, colcoords, value):
    ...
    if data.shape[0] < 2:
        raise ValueError("Data must have >1 row")
    if data.shape[1] < 2:
        raise ValueError("Data must have >1 col")
    if rowcoords.ndim != 1:
        raise ValueError("rowcoords must be 1D")
    if rowcoords.size != data.shape[0]:
        raise ValueError("rowcoords.size must = data.shape[0]")
    if colcoords.ndim != 1:
        raise ValueError("colcoords must be 1D")
    ...
    def rinterp_dn_to_y(r, c):
        ...
        return (
            rowcoords[r]
            + (rowcoords[r + 1] - rowcoords[r]) / (data[r + 1, c] - data[r, c]) * dv
        )
    ...
    def cinterp_rt_to_x(r, c):
        ...
        return (
            colcoords[c]
            + (colcoords[c + 1] - colcoords[c]) / (data[r, c + 1] - data[r, c]) * dv
        )
