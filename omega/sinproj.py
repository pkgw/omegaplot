...
_makeLatPainter = lambda ax: sphere.AngularAxisPainter(
    ax, 180.0 / np.pi, sphere.DISPLAY_DMS, sphere.WRAP_LAT
)

_lonstyles = {
    "lon": lambda ax: sphere.AngularAxisPainter(
        ax, 180.0 / np.pi, sphere.DISPLAY_DMS, sphere.WRAP_POS_DEG
    ),
    "ra": lambda ax: sphere.AngularAxisPainter(
        ax, 12.0 / np.pi, sphere.DISPLAY_HMS, sphere.WRAP_POS_HR
    ),
    "ha": lambda ax: sphere.AngularAxisPainter(
        ax, 12.0 / np.pi, sphere.DISPLAY_HMS, sphere.WRAP_ZCEN_HR
    ),
}
...
class SphereSinProjection(rect.RectCoordinates):
    ...
    def lin2arb(self, linx, liny):
        ...
        result[0] = self.lon0 + np.arctan2(
            linx * sinc, rho * cl0 * cosc - liny * sl0 * sinc
        )
        w = np.where(rho == 0)
