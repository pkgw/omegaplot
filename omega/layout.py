...
class Grid(Painter):
    ...
    def doLayout(self, ctxt, style, isfinal, w, h, bt, br, bb, bl):
        ...
        return LayoutInfo(
            minsize=(minw, minh), minborders=(vbord[0], hbord[-1], vbord[-1], hbord[0])
        )
...
class LinearBox(Painter):
    ...
    def __setitem__(self, idx, value):
        ...
        info.painter = value
        info.majsz = info.minsz = info.bmaj1 = info.bmaj2 = info.bmin1 = info.bmin2 = (
            0.0
        )
        info.aspect = None
    ...
    def _boxDoChildLayout(
        self, info, ctxt, style, isfinal, major, minor, bmaj1, bmin1, bmaj2, bmin2
    ):
        raise NotImplementedError()

    def _boxDoLayout(
        self, ctxt, style, isfinal, major, minor, bmaj1, bmin1, bmaj2, bmin2
    ):
        ...
        for i, e in enumerate(self._elements):
            if e.weight != 0:
                ...
                self._boxDoChildLayout(
                    e, ctxt, style, False, 0.0, minor, e.bmaj2, bmin1, e.bmaj2, bmin2
                )
        ...
        maxSPW = 0.0  # maximum size per weight.
        max_minor = max_bmin1 = max_bmin2 = 0.0
        ...
        for i, e in enumerate(self._elements):
            ...
            enforce_min_bounds(i, e)
            self._boxDoChildLayout(
                e, ctxt, style, isfinal, c_major, minor, e.bmaj1, bmin1, e.bmaj2, bmin2
            )
            enforce_min_bounds(i, e)
            ...
            if e.weight != 0:
                if e.aspect is not None:
                    raise RuntimeError(
                        "box items with fixed aspect ratios must have zero weights"
                    )
                fullreq = e.bmaj1 + e.bmaj2 + e.major
                maxSPW = max(maxSPW, 1.0 * fullreq / e.weight)
        ...
        return LayoutInfo(
            minsize=(major, max_minor), minborders=(bmaj1, max_bmin1, bmaj2, max_bmin2)
        )
...
class VBox(LinearBox):
    ...
    def _boxDoChildLayout(
        ...
        li = info.painter.layout(
            ctxt,
            style,
            isfinal,
            minor,
            major,  # w = minor, h = major
            bmaj1,
            bmin1,
            bmaj2,
            bmin2,
        ...
        if li.aspect is None:
            ...
            info.aspect = 1.0 / li.aspect  # box aspect is major/minor = h/w
    ...
    def doLayout(self, ctxt, style, isfinal, w, h, bt, br, bb, bl):
        ...
        return LayoutInfo(
            minsize=(li.minsize[1], li.minsize[0]), minborders=li.minborders
        )
