...
class AngularAxisPainter(rect.BlankAxisPainter):
    ...
    def _info(self, ctxt, style):
        ...
        for secincr, majorperminor, detaillev in _increments:
            ...
            axincr = secincr / (3600.0 * self.vscale)
            ...
            axspan_seconds = 3600.0 * self.vscale * axspan
            mip = int(np.floor(np.log10(1.0 * axspan_seconds)))  # major interval power
            secincr = 10**mip
        ...
        while self.axis.inbounds(axval):
            ...
            if info._label:
                ...
                mnt = int(np.floor(effsecval / 60 - unit * 60.0))
    ...
    def spaceExterior(self, helper, ctxt, style):
        ...
        for info in self._info(ctxt, style):
            ...
            outside = max(
                outside,
                helper.spaceRectOut(info.labelw, info.labelh, angle=self.angleLabels),
            )
            fw, bw = helper.spaceRectPos(
                info.xformed, info.labelw, info.labelh, angle=self.angleLabels
            )
    ...
    def paint(self, helper, ctxt, style):
        ...
        for info in infos:
            ...
            if not self.avoidBounds or (info.xformed != 0.0 and info.xformed != 1.0):
