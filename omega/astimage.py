...
# Support for plot axes derived from coordinates in an image
# loadable with my "astimage" module.
#
...
def _makeRAPainter(axis):
    return sphere.AngularAxisPainter(
        axis, 12.0 / np.pi, sphere.DISPLAY_HMS, sphere.WRAP_POS_HR
    )


def _makeLatPainter(axis):
    return sphere.AngularAxisPainter(
        axis, 180.0 / np.pi, sphere.DISPLAY_DMS, sphere.WRAP_LAT
    )


def _makeLonPainter(axis):
    return sphere.AngularAxisPainter(
        axis, 180.0 / np.pi, sphere.DISPLAY_DMS, sphere.WRAP_POS_DEG
    )


class AstimageCoordinates(rect.RectCoordinates):
    def __init__(self, image, field_or_plot):
        super(AstimageCoordinates, self).__init__(field_or_plot)
        self.image = image
        self.refworld = self.image.toworld(np.zeros_like(self.image.shape))
        self._sniff_discontinuities()

    def _sniff_discontinuities(self):
        """Check for discontinuities in the world coordinates as a function of pixel
        coordinates in the image area. We must postprocess the world
        coordinates to hide these so that all the code above us can keep on
        making assumptions that ought to be totally reasonable.
        ...
        90 degrees. The generalization of that issue is that world coordinates
        ...
        for ypix in 0, s[-2] - 1:
            ...
            while p[-1] < s[-1]:  # make our way across the image ...
                ...
                if expected_w is not None and abs((w1[-1] - expected_w) / w1[-1]) > 0.5:
                ...
                if abs(dlatdp) > 0.3 * abs(dlondp):
                    raise Exception(
                        "X axis does not (always) track the " "longitude axis closely"
                    )
    ...
    def makeAxis(self, side):
        ...
        if self.image.axdescs is None:
            axtype = None
        else:
            axtype = self.image.axdescs[axidx]

        if axtype.startswith("RA-"):
            axis.defaultPainter = _makeRAPainter
        elif axtype == "Right Ascension":
            axis.defaultPainter = _makeRAPainter
        elif axtype.startswith("DEC"):
            ...
            raise Exception(
                "Don't know what to do with axis of "
                "type " + (axtype or "<undefined>")
            )
